#!/bin/bash
#
# Z Image Turbo Setup for RunPod
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/Reuben-Fernandes/runpod-download/main/z-image-turbo.sh | HF_TOKEN=hf_yourtoken bash
#
...
if [[ -z "$HF_TOKEN" ]]; then
    ...
    echo "       curl -fsSL .../z-image-turbo.sh | HF_TOKEN=hf_yourtoken bash"
    exit 1
fi
echo "✓ Token detected"
...
download_hf "$MIRROR" "diffusion_models/z_image_turbo_bf16.safetensors" \
    "diffusion_models" "z_image_turbo_bf16.safetensors" || FAILED=1

download_hf "$MIRROR" "text_encoders/qwen_3_4b.safetensors" \
    "text_encoders" "qwen_3_4b.safetensors" || FAILED=1

download_hf "$MIRROR" "vae/ae.safetensors" \
...
echo ""
echo "########################################"
echo "#     Z Image Turbo Setup Complete    #"
echo "########################################"
echo ""
